...
import { getDayMusic, getPic } from "network/dayMusic.js";
export default {
  ...
  methods: {
    getDayMusicData() {
      getDayMusic().then((res) => {
        this.music = res.data.dailySongs;
      });
      setTimeout(() => {
        this.$refs.scroll.refresh();
      }, 88);
    ...
    getPicData() {
      getPic().then((res) => {
        this.picUrl = res.result[0].picUrl;
        setTimeout(() => {
          this.$refs.scroll.refresh();
        }, 88);
      });
